...
{
    public class LiquidationAssetController : BaseController
    {
        private readonly IFixedAssetHandler _fixedAssetHandler;

        private readonly IWarehouseHandler _warehouseHandler;

        public LiquidationAssetController( IFixedAssetHandler fixedAssetHandler, IWarehouseHandler warehouseHandler )
        {
        ...
        public ActionResult ListLiquidationAsset( )
        {
            GetData();
            TempData["Warehouse"] = GetCurrentWarehouseId();
            var data = _fixedAssetHandler.GetAllFixedAssets().Where ( n => n.FixedAssetType == FixedAssetType.LiquidationAsset && n.Center == GetCurrentUnitForUser() ).ToList();
            if ( data.Count > 0 )
                data = data.OrderByDescending ( x => x.CreatedDate ).ToList();
